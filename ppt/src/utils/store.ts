...
export const saveStudentsAnswer = (itemid: string, answer: string) => {
  localStorage.setItem(`${storeUid}_${STUDENTS_ANSWER}_${itemid}`, answer)
}

export const getStudentsAnswer = (itemid: string) => {
  const data = localStorage.getItem(`${storeUid}_${STUDENTS_ANSWER}_${itemid}`) || ''
...
export const saveTeacherAlist = (itemid: string, answerList: any[]) => {
  localStorage.setItem(`${storeUid}_${TEACHER_ANSWER}_${itemid}`, JSON.stringify(answerList))
}
export const getTeacherAlist = (itemid: string) => {
  let listString = localStorage.getItem(`${storeUid}_${TEACHER_ANSWER}_${itemid}`) || ''
  let arr = []
  try {
