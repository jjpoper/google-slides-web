...
export default class Draw {
  ...
  drawAndShareImage(str: string, text: string) {
    ...
    canvas.width = this.canvasWidth * 0.8;
    canvas.height = this.canvasHeight * 0.8;
    const context: any = canvas.getContext("2d");
    context.rect(0, 0, this.canvasWidth * 0.8, this.canvasHeight * 0.8);
    ...
    myImage.onload = () => {
      context.drawImage(myImage, 0, 0, this.canvasWidth * 0.8, this.canvasHeight * 0.8);
      const myImage2 = new Image();
      myImage2.src = text; // 你自己本地的图片或者在线图片
      myImage2.crossOrigin = 'Anonymous';
      myImage2.onload = () => {
        context.drawImage(myImage2, 0, 0, this.canvasWidth * 0.8, this.canvasHeight * 0.8);
