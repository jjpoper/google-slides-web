  ...
  <div class="mediaenter">
    ...
    >
      <div class="youtubecontaier">
        ...
        <el-input v-model="youtubeurl" placeholder="https://... Paste a youtube url here">
          <el-button slot="append" icon="el-icon-refresh" style="color: green" @click="youtubePreview"></el-button>
        </el-input>
    ...
    >
      <div class="showImageSearchcontaier">
        <div style="width: 50%">
          <el-input placeholder="请输入内容" v-model="imageName">
            <el-button slot="append" icon="el-icon-search" @click="searchImage"></el-button>
          </el-input>
        </div>
        <ul class="infinite-list" style="overflow:auto">
          <li @click="selectImage(index)"
            v-for="(item, index) in imagesList"
            :class="`infinite-list-item ${index === imageSelectedIndex ? 'imageSelected' : ''}`"
            :key="index">
            <div class="innerImage" :style="`background-image:url('${item.link}')`"></div>
          </li>
        </ul>
        <el-button-group style="margin-top:10px">
          <el-button type="primary" :disabled="imageSelectedIndex === -1" @click="doneSelect">select</el-button>
          <el-button type="info" style="margin-left:10px" @click="closeImageSearch">cancel</el-button>
        </el-button-group>
      </div>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    load() {
      showLoading()
      fetch(`https://www.googleapis.com/customsearch/v1?key=AIzaSyBwZ7igOKSCZ8nitWRvR_oZIO198pBs7jQ&cx=f0726c917433f216e&q=${this.imageName}&fileType=png,jpg&searchType=image&alt=json`,{
        headers: {
...
.youtubecontaier >button, .youtubecontaier > div {
  margin-bottom: 10px;
}
.iframeyoutube{
  width: 400px;
