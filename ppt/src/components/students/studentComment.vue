<template>
  <div v-if="modalVisibale" class="studentComment">
    <div class="title">
      <p>Teacher Feedback</p>
      <i @click="hidecomment"></i>
    </div>
    <div class="studentCommentList" :style="`height: ${webHeight}px; `" >
...
export default {
  ...
  computed: {
    ...
    commentList() {
      const list = [].concat(this.currentFeedList);
      list.sort((a, b) => {
        return a.id - b.id
      })
      list.sort((a) => {
        if(a.page_id === this.currentPageId){
          return -1
        }
        return 1
      })
